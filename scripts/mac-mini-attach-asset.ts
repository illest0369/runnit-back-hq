...
async function main() {
  ...
  const localAssetPath = readArg('--asset-path') ?? process.env.MAC_MINI_ASSET_PATH?.trim()
  if (!packageId) {
    throw new Error('Missing package id. Use --package-id <mac_mini_clip_package_id>.')
  }
  if (!localAssetPath) {
    throw new Error('Missing asset path. Use --asset-path <local_mp4_path>.')
  }

  const response = await fetch(`${apiBaseUrl()}/api/mac-mini/packages/${encodeURIComponent(packageId)}/asset`, {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
      'x-rbhq-mac-mini-token': workerToken(),
    },
    body: JSON.stringify({ localAssetPath }),
  })
  const body = await response.json().catch(() => null)
  if (!response.ok) {
    throw new Error(`Mac mini asset attach failed: ${response.status} ${JSON.stringify(body)}`)
  }
