...
function loadSportsSeed(): SeedEntry[] {
  ...
  return seed.filter((entry) => entry.lane_key === 'sports')
}
...
async function loadDbSources() {
  const { data, error } = await createSupabase()
    ...
    .eq('target_rbhq_channel_id', RB_SPORTS_CHANNEL_ID)
    .order('display_name')
...
function compareSources(seed: SeedEntry[], dbSources: SourceRow[]) {
  const activeConfig = rbSportsPhase1ActiveSources()
  const noisyConfig = rbSportsPhase1NoisySources()
  ...
  return {
    ...
    categoriesCovered: {
      officialLeagueHighlightsNews: activeCategories.has('official_league_highlights_news'),
      officialTeamChannel: activeCategories.has('official_team_channel'),
      trustedBroadcastHighlight: activeCategories.has('trusted_broadcast_highlight'),
      trustedSportsMediaAnalysis: activeCategories.has('trusted_sports_media_analysis'),
      disabledDiscoveryFanReaction: noisyConfig.some((source) => source.category === 'discovery_fan_reaction' && !source.active),
    },
...
async function syncSources(seed: SeedEntry[]) {
  ...
  const configByKey = new Map(RB_SPORTS_PHASE1_SOURCES.map((source) => [source.channelKey, source]))
  const synced: string[] = []
  ...
  for (const entry of seed) {
    ...
    if (!configSource) {
      skipped.push({ channelKey: entry.channel_key, reason: 'Not in RB Sports Phase 1 source config.' })
      continue
    }
  ...
  const activeKeys = rbSportsPhase1ActiveSources().map((source) => source.channelKey)
