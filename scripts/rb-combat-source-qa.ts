...
function loadCombatSeed(): SeedEntry[] {
  ...
  return seed.filter((entry) => entry.lane_key === 'combat')
}
...
async function loadDbSources() {
  const { data, error } = await createSupabase()
    ...
    .eq('target_rbhq_channel_id', RB_COMBAT_CHANNEL_ID)
    .order('display_name')
...
function compareSources(seed: SeedEntry[], dbSources: SourceRow[]) {
  const activeConfig = rbCombatPhase1ActiveSources()
  const noisyConfig = rbCombatPhase1NoisySources()
  ...
  const activeCategories = new Set(activeConfig.map((source) => source.category))
  ...
  return {
    ...
    categoriesCovered: {
      officialPromotionChannels: activeCategories.has('official_promotion_channel'),
      officialLeagueEventChannels: activeCategories.has('official_league_event_channel'),
      trustedBroadcastHighlightChannels: activeCategories.has('trusted_broadcast_highlight'),
      trustedCombatMediaInterviewChannels: activeCategories.has('trusted_combat_media_interview'),
      disabledDiscoveryReactionSources: noisyConfig.some((source) => source.category === 'discovery_fighter_reaction' && !source.active),
      disabledNoisyWatchlists: noisyConfig.some((source) => source.category === 'disabled_noise' && !source.active),
    },
...
async function syncSources(seed: SeedEntry[]) {
  ...
  const configByKey = new Map(RB_COMBAT_PHASE1_SOURCES.map((source) => [source.channelKey, source]))
  const synced: string[] = []
  ...
  for (const entry of seed) {
    ...
    if (!configSource) {
      skipped.push({ channelKey: entry.channel_key, reason: 'Not in RB Combat Phase 1 source config.' })
      continue
    }
  ...
  const activeKeys = rbCombatPhase1ActiveSources().map((source) => source.channelKey)
