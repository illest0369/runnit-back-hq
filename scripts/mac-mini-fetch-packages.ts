...
async function main() {
  ...
  const response = await fetch(`${apiBaseUrl()}/api/mac-mini/packages?limit=${encodeURIComponent(limit)}`, {
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Mac mini package fetch failed: ${response.status} ${JSON.stringify(body)}`)
  }
